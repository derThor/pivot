...
- Kein Backend-Change nötig – `GET/POST /users` und `PATCH /users/:id`
  ...
  [auth-jwt-refresh-rotation.md](./auth-jwt-refresh-rotation.md), RBAC-Teil).
...
- Keine – deckungsgleich mit dem bereits etablierten BFF-Route-Handler-
  Muster aus [content-editor-dynamic-forms.md](./content-editor-dynamic-forms.md).
...
- Kein Deaktivieren/Löschen von Benutzern in der UI (Backend-Endpoint
  `DELETE /users/:id` existiert, `isActive` per `PATCH` theoretisch auch,
  aber nicht Teil dieses Roadmap-Punkts) – siehe
  [`docs/ROADMAP.md`](../docs/ROADMAP.md) Phase 2. Löschen muss dabei
  über [`ConfirmDeleteDialog`](./ui-convention-crud-and-delete-confirmation.md)
  laufen.
