...
- Rollenbeschränkung: über das granulare RBAC-System (`content:create`/
  `update`/`delete`/`read`), siehe [rbac-rework.md](../auth/rbac-rework.md).
  ~~Lesen ist für alle authentifizierten Rollen offen~~ – seit
  [read-permissions-and-dashboard-access.md](../auth/read-permissions-and-dashboard-access.md)
  ist auch Lesen an das Recht `content:read` gebunden.
...
- ~~Versions-Diff & Rollback-Endpoint/UI~~ – inzwischen erledigt, siehe
...
- ~~Kategorien/Tags haben zwar ein Datenmodell, aber noch keine eigenen
  Endpoints/DTOs~~ – inzwischen erledigt, siehe
  [taxonomy-management.md](../frontend/taxonomy-management.md).
- Scheduler für `SCHEDULED` → `PUBLISHED` fehlt (siehe
  [`docs/ROADMAP.md`](../../docs/ROADMAP.md) Phase 2b.3).
