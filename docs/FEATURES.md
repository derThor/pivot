...
| Passwort-Reset per E-Mail | ✅ | Echter SMTP-Versand seit 2026-08-22 (sonst Dev-Stub-Log ohne konfigurierten Mailserver), abschaltbar per Einstellung |
| E-Mail-Verifikation | ✅ | Blockiert Login nicht, nur Dashboard-Hinweisbanner; echter SMTP-Versand seit 2026-08-22 |
...
| Draft/Publish/Scheduled/Archived-Workflow | ✅ | Scheduler für `SCHEDULED` → `PUBLISHED` läuft minütlich, Zeitplan editierbar unter Einstellungen → Jobs |
...
| Einstellungen-UI | ✅ | 8-Bereiche-Sidebar (Zugriff & Funktionen, Sicherheit, Benachrichtigungen, Darstellung, Integrationen, Webhooks, Jobs, Protokoll – Firma/Datenschutz leben seit 2026-08-17/18 als eigene Seiten unter Verwaltung, nicht mehr hier): Passwort-Policy inkl. Leak-Prüfung/Wiederverwendungs-Sperre, dreistufige 2FA-Pflicht (aktiviert alle Sitzungen sofort neu, nicht erst nach bis zu 15 Min.), Sitzungs-Inaktivitäts-Timeout, "Alle Sitzungen beenden" (meldet auch die eigene Sitzung sofort ab), Feature-Schalter (Wartungsmodus, Medien-Speicherkontingent, maximale Upload-Dateigröße), gemeinsamer Benachrichtigungsempfänger für Systembenachrichtigungen, Darstellung (Akzentfarbe, Tabellendichte, Logo), Integrationen → Dienste (echter SMTP-Mailversand samt Testmail/Verbindungstest), Jobs (die 3 echten Cron-Jobs der App zur Laufzeit editierbar, Lauf-Historie, Pause/kritisch-Schalter), Protokoll (Änderungshistorie + CSV/JSON-Export); Sidebar-Logo und Auth-Bild sind fest hinterlegt, nicht mehr konfigurierbar |
...
| Datenschutz-Seite (`/dashboard/privacy`) | ✅ | Eigene Seite unter Verwaltung: Rechtstexte aus den Firmen-Stammdaten generiert (inkl. Verknüpfung zu einer echten Content-Seite), Betroffenenrechte-Karte (Auskunft nach Art. 15 DSGVO erstellen/per Mail senden, AV-Verträge als ZIP herunterladen), Aufbewahrungsfristen als Richtwerte + manuelle Löschen-Review-Listen (kein Auto-Löschen), Betroffenenanfragen-Log (Liste+Detail, DSR-IDs, Löschung/Auskunft/Berichtigung, Datenauszug/Löschbestätigung/Rückfrage-Popup, Erinnerungs-Mails), Auftragsverarbeiter-Tab (Liste+Detail: Ort/Zusatzhinweis/Drittlandtransfer, "Offene Punkte" mit AV-Vertrag-Anfrage-Mail, SCC-Vorlagen-Upload/-Download), Verarbeitungsverzeichnis/Vorfälle (einfache CRUD-Listen), Datenschutzbeauftragter-Kontakt (inkl. Erwähnung in Rechtstexten, Vorfall-Benachrichtigung, monatlicher Bericht per Mail), CSV-Compliance-Bericht (deutsche Texte, UTF-8-BOM) |
...
| Papierkorb (`/dashboard/trash`) | ✅ | Vereinheitlichte Papierkorb-Seite für Seiten/Medien/Kategorien/Tags/Galerien/FAQs/Formulare (Bausteine/Modul-Typen bewusst ausgenommen, sind Vorlagen), eigener Sidebar-Eintrag unter Webseite: Löschen verschiebt jetzt app-weit nur noch in den Papierkorb, Stat-Kacheln, Ablauf-Warnbanner mit Sammel-Wiederherstellung, Filter mit Zählern/Suche, Massenauswahl, farbige Typ-Icons/Badges + Detail-Zeile pro Eintrag; nach Ablauf der Aufbewahrungsfrist nur Sperrung der Wiederherstellung, kein automatisches Löschen |
| Formulare (`/dashboard/forms`) | ✅ | Eigener Feld-Builder (Basis/Auswahl/Erweitert), Status Entwurf/Live/Pausiert, Seiten-Designer-Baustein „Formular" (öffentlich absendbar über `POST /forms/:slug/submit`), Einsendungen pro Formular + app-weite Sammelübersicht (gelesen/gelöscht), „Abgelaufen"-Badge nach der Datenschutz-Aufbewahrungsfrist (Löschen bleibt manuell). Details: [forms.md](../knowledge-base/content/forms.md) |
| Mailing (Einstellungen → Mailing) | ✅ | Eine gemeinsame, editierbare Vorlagen-Liste für alle System-Mails UND die beiden Vorlagen jedes Formulars (Admin-Benachrichtigung + Bestätigung an Absender); pro Vorlage Betreff/Text, Platzhalter-Chips, Testmail-Versand, Vorschau, „Versand aktiv"-Schalter, „Auf Standard zurücksetzen". Weiterhin reiner Text, kein HTML-Layout. Details: [forms.md](../knowledge-base/content/forms.md) |
